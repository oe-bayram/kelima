...
2. Admin-Nutzer anlegen und der Gruppe zuordnen (Pool-ID steht in `amplify_outputs.json`
   ...
   - Registrieren/bestätigen über die App (Authenticator) **oder**:
     ```bash
     aws cognito-idp admin-create-user --user-pool-id <POOL_ID> --username oebayram@gmail.com
     aws cognito-idp admin-set-user-password --user-pool-id <POOL_ID> --username oebayram@gmail.com --password '<PW>' --permanent
     ```
   - Der Gruppe `admin` zuordnen:
     ```bash
     aws cognito-idp admin-add-user-to-group --user-pool-id <POOL_ID> --username oebayram@gmail.com --group-name admin
     ```
3. Für die Negativtests zusätzlich einen **Nicht-Admin**-Testnutzer anlegen (nicht in `admin`).

## Ausführen

> Das Skript heißt bewusst **`import:data`** (nicht `import`): `pnpm import` ist ein
> **eingebautes pnpm-Kommando** und würde die `pnpm-lock.yaml` überschreiben statt das
> Skript auszuführen. Alternativ funktioniert immer `pnpm run import:data`.
...
KELIMA_ADMIN_EMAIL=oebayram@gmail.com KELIMA_ADMIN_PASSWORD='<PW>' pnpm import:data -- --file data/wortliste_b1_struktur.json --version 2026-07-04

# Nur einzelne Modelle schreiben:
pnpm import:data -- --only forms
pnpm import:data -- --only entries,examples
```

Auf Windows/PowerShell die Env-Vars vorher setzen:

```powershell
$env:KELIMA_ADMIN_EMAIL="oebayram@gmail.com"; $env:KELIMA_ADMIN_PASSWORD="<PW>"; pnpm import:data -- --version 2026-07-04
```
